#!/usr/bin/env python3
"""Execute the pre-registered v0.6 learned-macro-geometry holdout."""
...
ROOT=Path(__file__).resolve().parent;OUT=ROOT/'artifacts'/'v06'
PROTOCOL=ROOT/'protocol_v06.json';HOLDOUT=ROOT/'holdout_v06.json'


def sha(path):return hashlib.sha256(path.read_bytes()).hexdigest()


def evaluate(model,inverse,templates,maps,limit):
    writes=model.writes;output={}
    for item in maps:
        level=parse(item['map']);started=time.perf_counter();result=solve(level,model,inverse,templates,limit)
...
def main():
    ...
    assert json.loads(json.dumps(templates))==protocol['expected_templates']
    assert evidence==protocol['expected_template_evidence'] and pushes==protocol['expected_template_training_pushes']
    ...
    result={'format':'bpc-learned-geometry-v06','evidence_level':'developer-frozen local holdout; not third-party blind',
        'training_experience':experience,'training_manifest':manifest,'model_sha256':model.digest(),
        'inverse':inverse,'inverse_evidence':inverse_evidence,'templates':templates,
        'template_evidence':evidence,'template_training_pushes':pushes,'template_audit':audit,
