...
def same_cell_support_compatible(reference,target,mapping):
    """Require both directional conditional event alphabets to be preserved."""
    for (a,b),(rab,rba) in reference.items():
        oa,ob=mapping[a],mapping[b];ab,ba=target[tuple(sorted((oa,ob)))]
        if oa>ob:ab,ba=ba,ab
        if set(ab)!=set(rab) or set(ba)!=set(rba):return False
    return True
...
def infer_relational_support_binding(reference,reference_support,reference_triples,target_rows):
    ...
    prepared=[(stats,support,same_cell_counts(triples,stats.channels)) for stats,support,triples in target_rows];rows=[];change_compatible=0
    for mapping in itertools.permutations(range(prepared[0][0].channels),reference.channels):
        if all(projected_support(support,mapping)==reference_support for _,support,_ in prepared):
            change_compatible+=1
            if not all(same_cell_support_compatible(base,pairs,mapping) for _,_,pairs in prepared):continue
    ...
    if not rows:return {'mapping':None,'change_compatible':change_compatible,'compatible':0,'per_stream':()}
    rows.sort();top=rows[-1][0]
    if len(rows)==1:return {'mapping':rows[0][1],'posterior':1.,'log_margin':float('inf'),'second':None,
        'change_compatible':change_compatible,'compatible':1,'per_stream':()}
    ...
    return {'mapping':winner[1],'posterior':1/z,'log_margin':winner[0]-rows[-2][0],'second':rows[-2][1],
        'change_compatible':change_compatible,'compatible':len(rows),'sensor_log_probability':sum(x[0] for x in winner[2]),
