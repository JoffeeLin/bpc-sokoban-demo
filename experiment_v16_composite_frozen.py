#!/usr/bin/env python3
"""Execute the pre-registered v0.16 composite-interface transfer once."""
import hashlib,inspect,json,random,re,time
from pathlib import Path

from bpc_composite_binding_v16 import (CompositeBoundPolicy,changed,collect_composite_interface,
    evaluate_composite,infer_support_binding,projected_support)
from bpc_open_interface_v15 import (OpenActionStats,OpenInterfaceStats,action_stats,infer_action_subset,inverse_subset)
from bpc_three_factor_v12 import World3,key,shortest,train

ROOT=Path(__file__).resolve().parent;PROTOCOL=ROOT/'protocol_v16_composite.json';HOLDOUT=ROOT/'holdout_v16.json';OUT=ROOT/'artifacts'/'v16composite'
...
def main():
    ...
    for index,(seeds,sensors,actuators,expected) in enumerate(zip(config['target_seeds'],config['sensor_routes'],config['actuator_routes'],protocol['expected_calibration'])):
        sensors=tuple(sensors);actuators=tuple(actuators);targets=[];triples=[];regimes=[]
        ...
        assert target_actions.digest()==expected['target_action_digest']
        assert list(inverse_subset(sensors,6))==expected['channel_oracle'] and list(inverse_subset(actuators,4))==expected['action_oracle']
        interfaces.append({'index':index,'sensors':sensors,'actuators':actuators,'channel':channel,'action':action,'regimes':regimes,
    ...
    for interface in interfaces:
        random_channels=rng.sample(range(len(interface['sensors'])),6);random_actions=rng.sample(range(len(interface['actuators'])),4)
        policies={'learned':CompositeBoundPolicy(learner,interface['channel']['mapping'],len(interface['sensors']),interface['action']['mapping'],len(interface['actuators'])),
            'oracle':CompositeBoundPolicy(learner,interface['channel_oracle'],len(interface['sensors']),interface['action_oracle'],len(interface['actuators'])),
            'sensor_only':CompositeBoundPolicy(learner,interface['channel']['mapping'],len(interface['sensors']),range(4),len(interface['actuators'])),
            'action_only':CompositeBoundPolicy(learner,range(6),len(interface['sensors']),interface['action']['mapping'],len(interface['actuators'])),
            'identity':CompositeBoundPolicy(learner,range(6),len(interface['sensors']),range(4),len(interface['actuators'])),
            'random':CompositeBoundPolicy(learner,random_channels,len(interface['sensors']),random_actions,len(interface['actuators']))}
        by_seed={};by_trace={}
        for seed in config['eval_seeds']:
            result,traces=evaluate_composite(policies,suite,interface['sensors'],interface['actuators'],seed,config['episodes_per_map'],config['step_budget'])
    ...
    source='\n'.join(inspect.getsource(x) for x in (changed,projected_support,infer_support_binding,OpenInterfaceStats.observe,
        OpenActionStats.observe,infer_action_subset,CompositeBoundPolicy.probabilities))
    ...
    gate={'four_unseen_composite_sensor_subsets_recovered':all(tuple(x['channel']['mapping'])==x['channel_oracle'] for x in interfaces),
        ...
        'exactly_four_support_compatible_bases_per_interface':all(x['channel']['compatible']==threshold['compatible_mappings'] for x in interfaces),
        'combined_sensor_log_margin_at_least_10000':all(x['channel']['log_margin']>=threshold['combined_sensor_log_margin'] for x in interfaces),
        'each_stream_sensor_log_margin_at_least_1000':all(all(row['log_margin']>=threshold['per_stream_sensor_log_margin'] for row in x['channel']['per_stream']) for x in interfaces),
        ...
        'both_subsets_beat_identity_by_45pp':(joint['learned']-joint['identity'])/joint_episodes>=threshold['identity_joint_margin'],
        'both_subsets_beat_random_by_45pp':(joint['learned']-joint['random'])/joint_episodes>=threshold['random_joint_margin'],
    ...
    result={'format':'bpc-composite-binding-v16','evidence_level':'developer-frozen held-out synthetic composite-interface transfer; not third-party blind',
        'question':protocol['question'],'training':events,'factor_digest':learner.digest(),
        'calibration':{'reference_interface_digest':reference.digest(),'reference_support':support(reference_support),'reference_action_digest':reference_actions.digest(),
            'interfaces':[{'sensor_routes':list(x['sensors']),'actuator_routes':list(x['actuators']),'regimes':x['regimes'],
                'channel_oracle':list(x['channel_oracle']),'channel_binding':norm_sensor(x['channel']),
                'action_oracle':list(x['action_oracle']),'action_binding':norm_action(x['action']),
        ...
        'supported_claim':'A frozen non-neural BPC controller identified its canonical six-sensor/four-action interface inside four simultaneous unseen eight-channel/six-slot interfaces with displaced-XOR composite sensor distractors and null action slots. Empirical transition-support invariance reduced each sensor assignment space from 20,160 to four compatible bases; probabilities across four separately sampled unlabeled regimes selected the oracle basis and preserved oracle direct control on disjoint unseen worlds.',
