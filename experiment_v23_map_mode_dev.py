...
def main():
    ...
    for index,row in enumerate(protocol['expected_calibration']):
        ...
        conditions[str(index)]=evaluate_map(learner,profiles,learned,suite,SENSORS[index],KINDS[index],SPATIAL[index],DELAYS[index],tuple(row['sensor_binding']['mapping']),
            row['selected_gauge_representative'],CHANNELS[index],oracle,TRANSITIONS[index],INITIALS[index],216010+index,8,128)
    per=[]
    for key,row in conditions.items():per.append({'interface':int(key),'episodes':row['map']['episodes'],'joint_episodes':64,
        **{name:value['successes'] for name,value in row.items()},**{f'{name}_joint':value['joint_successes'] for name,value in row.items()}})
    gates={'each_map_at_least_average_overall':all(x['map']>=x['average'] for x in per),
        'failed_interface_2_map_beats_average_joint_by_5pct':per[2]['map_joint']-per[2]['average_joint']>=per[2]['joint_episodes']*.05,
        'failed_interface_2_map_beats_memoryless_joint_by_5pct':per[2]['map_joint']-per[2]['memoryless_joint']>=per[2]['joint_episodes']*.05,
        'each_map_beats_shuffled_joint_by_5pct':all(x['map_joint']-x['shuffled_joint']>=x['joint_episodes']*.05 for x in per),
        'evaluation_writes_zero':learner.writes==writes}
    result={'development_only':True,'v21_frozen_result_was_non_adopted':True,'mechanism':'posterior MAP mode directly selects the actuator inverse; filtering remains Bayesian',
        'conditions':conditions,'diagnostic':{'per_interface':per},'preregistered_gates':gates,'adopt_for_v23_freeze':all(gates.values()),
        'classifier_dev':{'model':'jev-1.13.0','selected':'posterior MAP hidden mode direct decision','confidence':.32,'role':'low-confidence route classification only'},
    ...
    destination=ROOT/'artifacts'/'v23map'/'development.json';destination.parent.mkdir(parents=True,exist_ok=True);destination.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
